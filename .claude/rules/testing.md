...
- Format: prefix + hex length asserted. Per-surface helpers emit `fallow:<kind>:<8 hex>`; `FunctionIdentity::stable_id` emits `fallow:fn:<16 hex>`; the hash inputs for both recipes are documented under "Key design decisions" in `CLAUDE.md`.
